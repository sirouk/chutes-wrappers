...
def build_wrapper_image(username: str, name: str, tag: str, base_image: str) -> Image:
    return (
        ...
        .run_command(
            ...
            "        seen.add(real)\n"
            "        paths.append(real)\n"
            "def looks_like_pkg(path: pathlib.Path):\n"
            "    if not path.is_dir():\n"
            "        return False\n"
            "    markers = ['__init__.py', 'setup.py', 'pyproject.toml', 'setup.cfg']\n"
            "    return any((path / marker).exists() for marker in markers) or any(path.glob('*.py'))\n"
            "for base in hints:\n"
            "    if not base.exists():\n"
            "        continue\n"
            "    add(base)\n"
            ...
            "            add(child)\n"
            "pth_path = pathlib.Path(site.getsitepackages()[0]) / 'chutes_app_path.pth'\n"
            "if paths:\n"
            "    pth_path.write_text('\\n'.join(paths) + '\\n')\n"
            "PY\n"
        )
